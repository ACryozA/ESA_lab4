...
Все описанные компоненты интегрированы в единый контур: изменение сущности вызывает отправку события в JMS, затем сообщение принимается, валидируется и записывается в таблицу `log_entity`, а при удалении сущности дополнительно отправляется email‑уведомление. В результате приложение одновременно ведёт журнал изменений в базе данных и автоматически оповещает по электронной почте о критичных операциях удаления.
